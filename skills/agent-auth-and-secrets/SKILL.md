...
- Hand-rolling header maps where the LLM controls a header value. Lock the entire `Authorization` header via `argument_override.headers.Authorization` with `$ref`; let the LLM only fill what's safe.
- Putting structured user-supplied fields like `requestor_email` into `web_get.body`. The body is a string the LLM constructs; field-level config injection only works on `headers` and `url`. For structured-arg tools use `mcp` or `dynamic_vectara` tool types.
