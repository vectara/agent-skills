...
| [queries](skills/queries/SKILL.md) | Direct `/v2/query` calls — single/multi-corpus, hybrid search, filters, rerankers (chain, knee, MMR, multilingual, UDF), citations, the optional `generation` block (presets, Velocity prompts, BYO LLM). Same shape applies inside an agent's `corpora_search` tool. |
